# ==============================================================================
# views/dashboard_view.py - Complete Dashboard Tab View
...
def setup_dashboard_tab(app, tab):
    ...
    # FIXED: Added padx=0 for vertical alignment with header
    ...
    # FIXED: Added padx=0 for vertical alignment
    hw_card = StandardCard(left_container, title="SYSTEM TELEMETRY", icon="📊")
    hw_card.pack(fill="x", padx=0)
    
    def create_v_meter(parent, label_var_name, bar_var_name, default_text="Loading..."):
        ...
        lbl = ctk.CTkLabel(tile, text=default_text, font=("Segoe UI", 11, "bold"), text_color="#E2E8F0")
    ...
    create_v_meter(hw_card, "lbl_cpu_val", "bar_cpu", "CPU Usage: 0%")
    create_v_meter(hw_card, "lbl_ram_val", "bar_ram", "RAM: 0G / 0G")
    create_v_meter(hw_card, "lbl_disk_val", "bar_disk", "Disk: 0G / 0G")
    ...
    app.btn_install_server = ctk.CTkButton(
        install_layout, 
        text="INSTALL / UPDATE", 
        width=140, 
        height=28, 
        ...
        command=app.trigger_steamcmd_installation
    )
